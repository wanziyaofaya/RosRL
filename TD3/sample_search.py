...
import math
from utils import Planner, Map, Node
...
class SampleSearcher(Planner):
    ...
    Parameters:
        ...
        env (Map): environment
    """
    def __init__(self, start: tuple, goal: tuple, env: Map, delta: float=0.05) -> None:
    ...
    def isCollision(self, node1: Node, node2: Node) -> bool:
        ...
        """
        # print(f"Collision check between {node1.current} and {node2.current}")  # Debugging: Collision check
        ...
        # print("No collision detected")  # Debugging: No collision
        return False
    ...
    def isInsideObs(self, node: Node) -> bool:
        ...
        for (ox, oy, w, h) in self.env.boundary:
            if not (0 <= x - (ox - self.delta) <= w + 2 * self.delta \
                and 0 <= y - (oy - self.delta) <= h + 2 * self.delta):
                return True
