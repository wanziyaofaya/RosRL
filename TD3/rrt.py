...
from utils import Node, Map, Planner
...
class RRT(SampleSearcher):
    ...
    def plan(self) -> tuple:
        ...
        for _ in range(self.sample_num):
            ...
            # Debugging: Check random node validity
            # print(f"Random Node: {node_rand.current}")
            ...
            # print(f"New Node: {node_new.current if node_new else 'None'}")  # Debugging: New node
            if node_new:
    ...
    def generateRandomNode(self) -> Node:
        ...
        if np.random.random() > self.goal_sample_rate:
            ...
            current = (np.random.uniform(x_min + self.delta, x_max - self.delta),
                       np.random.uniform(y_min + self.delta, y_max - self.delta))
